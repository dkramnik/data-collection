...
DCR_dark = TCR_dark;
p_ap_fast_dark = TCR_dark;

for j = 1 : length( holdoff_list )
    fig_handle = figure( );
    
    for i = 1 : length( VA_list )
        [ TCR_dark( i, j ), DCR_dark( i, j ), p_ap_fast_dark( i, j ) ] = ...
...
p_ap_hist = zeros( size( p_ap_fast_dark ) );
...
DCR_light = TCR_light;
p_ap_fast_light = TCR_light;

for j = 1 : length( holdoff_list )
    fig_handle = figure( );
    
    for i = 1 : length( VA_list )
        [ TCR_light( i, j ), DCR_light( i, j ), p_ap_fast_light( i, j ) ] = ...
            SRA_make_plot( raw_data{ i, j }, true, fig_handle, [ ] );
    end
end
...
p_ap_hist = zeros( size( p_ap_fast_light ) );
...
grid on;
for i = 1 : length( holdoff_list )
    plot( VA_list - V_BR, 1e-3 * DCR_dark( :, i ), 'o--' );
end
legend( legend_strings, 'location', 'NW' );
